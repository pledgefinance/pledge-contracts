...
async function main() {
    ...
    await txMined(notional.escrow.deposit(daiToken.address, parseEther("500")));
    await txMined(notional.escrow.withdraw(daiToken.address, parseEther("1000")));

    log("Adding $1,000 liquidity to 1M Dai market");
    await initializeLiquidity(1, notional, account, 0, parseEther("1000"), parseEther("1000"));
