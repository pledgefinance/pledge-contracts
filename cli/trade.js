...
async function batchOperation(deposits, trades, withdraws, maxTime, web3) {
  ...
  if (withdraws.length == 0) {
    await tradeContract.methods.batchOperation(web3.eth.defaultAccount, maxTime, deposits, trades).send(gasOptions(web3)).on (
    ...
    await tradeContract.methods.batchOperationWithdraw(web3.eth.defaultAccount, maxTime, deposits, trades, withdraws).send(gasOptions(web3)).on(
      'receipt', function(receipt) {
...
// Amount should already have been converted to appropriate decimals
function constructTrade(tradeType, marketAddress, maturity, amount, minRate, maxRate, maxfCash) {
  return {
