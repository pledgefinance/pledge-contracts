...
async function initializeLiquidity(cashGroup: number, notional: NotionalDeployer, account: Wallet) {
    ...
    const trace = await (notional.provider as JsonRpcProvider).send("debug_traceTransaction", [
        txn.transactionHash,
        {},
    ]);
...
async function logTrace(trace: any) {
    ...
    for (let line of trace.structLogs) {
        ...
        let tmpMem = Array.from(line.memory)
            .map((v) => new BigNumber("0x" + (v as string)).toString())
            .toString();
        let tmpStack = Array.from(line.stack)
            .map((v) => new BigNumber("0x" + (v as string)).toString())
            .toString();
