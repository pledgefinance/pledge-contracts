...
describe("ERC1155 Token", () => {
    ...
    describe("batch operations", async () => {
        it("reverts trade on maxTime", async () => {
            await expect(erc1155trade.connect(wallet).batchOperation(wallet.address, 0, [], [])).to.be.revertedWith(
                ErrorDecoder.encodeError(ErrorCodes.TRADE_FAILED_MAX_TIME)
            );
            await expect(
        ...
        it("reverts trade on invalid currency deposit", async () => {
            await expect(
                erc1155trade
                    .connect(wallet)
                    .batchOperation(wallet.address, BLOCK_TIME_LIMIT, [{currencyId: 3, amount: parseEther("100")}], [])
            ...
            await expect(
                erc1155trade
                    ...
                    .batchOperationWithdraw(
                        ...
                        [],
                        [],
                        [{to: owner.address, currencyId: 3, amount: parseEther("100")}]
                    )
            ).to.be.revertedWith(ErrorDecoder.encodeError(ErrorCodes.INVALID_CURRENCY));
        });
        ...
        it("allows trade [deposit]", async () => {
            await erc1155trade
                .connect(wallet2)
                .batchOperation(wallet2.address, BLOCK_TIME_LIMIT, [{currencyId: 1, amount: parseEther("100")}], []);
        ...
        it("reverts trade on slippage [deposit, takefCash, slippageData]", async () => {
            ...
            await expect(
                erc1155trade.connect(wallet2).batchOperation(
                    wallet2.address,
                    BLOCK_TIME_LIMIT,
                    [{currencyId: 1, amount: parseEther("100")}],
                    [
        ...
        it("reverts on fc check [deposit, takeCurrentCash, withdraw]", async () => {
            await expect(
                erc1155trade.connect(wallet2).batchOperationWithdraw(
                    wallet2.address,
                    BLOCK_TIME_LIMIT,
                    [{currencyId: 0, amount: parseEther("1.5")}],
                    [
        ...
        it("allows trade [deposit, addLiquidity, slippageData(min/max rate, futureCash)]", async () => {
            const slippage = defaultAbiCoder.encode(
                ["uint32", "uint32", "uint128"],
                [0, 100_000_000, parseEther("100")]
            );
            await erc1155trade.connect(wallet2).batchOperation(
                wallet2.address,
                BLOCK_TIME_LIMIT,
        ...
        it("reverts trade on slippage [deposit, addLiquidity, slippageData(min/max rate, fCash)", async () => {
            ...
            slippage = defaultAbiCoder.encode(
                ["uint32", "uint32", "uint128"],
                [100_000_000, 200_000_000, parseEther("100")]
            );
            await expect(
                erc1155trade.connect(wallet2).batchOperation(
                    wallet2.address,
        ...
        it("allows deposit for batchOperation when undercollateralized", async () => {
            ...
            await erc1155trade
                .connect(wallet2)
                .batchOperation(wallet2.address, BLOCK_TIME_LIMIT, [{currencyId: 1, amount: parseEther("1")}], []);
    ...
    describe("block trades", async () => {
        ...
        it("allows the bridge proxy to call the function", async () => {
            ...
            await expect(
                erc1155trade
                    .connect(wallet)
                    .safeTransferFrom(wallet.address, wallet2.address, cashPayerAssetId, parseEther("100"), data)
            ).to.be.revertedWith(ErrorDecoder.encodeError(ErrorCodes.UNAUTHORIZED_CALLER));
        });
        ...
        it("operators must be approved by both from and to (or from == msg.sender)", async () => {
            ...
            await expect(
                erc1155trade
                    .connect(wallet2)
                    .safeTransferFrom(wallet.address, owner.address, cashPayerAssetId, parseEther("100"), data)
            ).to.be.revertedWith(ErrorDecoder.encodeError(ErrorCodes.UNAUTHORIZED_CALLER));
            ...
            await expect(
                erc1155trade
                    .connect(wallet2)
                    .safeTransferFrom(owner.address, wallet.address, cashPayerAssetId, parseEther("100"), data)
            ).to.not.be.reverted;
        });
        ...
        it("reverts on liquidity token trade", async () => {
            const id = await erc1155trade["encodeAssetId(uint8,uint16,uint32,bytes1)"](
                ...
                0,
                maturities[0],
                AssetType.LIQUIDITY_TOKEN
            );
            await expect(
                erc1155trade.safeTransferFrom(wallet.address, wallet2.address, id, parseEther("100"), "0x")
        ...
        it("reverts idiosyncratic trade on over max length", async () => {
            ...
            await expect(
                erc1155trade.safeTransferFrom(wallet.address, wallet2.address, id, parseEther("100"), data)
            ).to.be.revertedWith(ErrorDecoder.encodeError(ErrorCodes.PAST_MAX_MATURITY));
        });
        ...
        it("idiosyncratic assets do not net out in the risk formula", async () => {
            ...
            expect((await portfolios.freeCollateralView(wallet2.address))[1][CURRENCY.DAI]).to.equal(
                parseEther("-100")
            );
        });
    });
