...
describe("Liquidation", () => {
    ...
    describe("settle cash, local currency scenarios [1-3]", async () => {
        it("[1] should not do anything if the value is set to 0", async () => {
            await escrow.connect(wallet2).settleCashBalance(CURRENCY.DAI, CURRENCY.ETH, owner.address, 0);
        });
        ...
        it("[1] should settle not cash between accounts when there is insufficient cash balance", async () => {
            ...
            await expect(
                escrow.settleCashBalance(CURRENCY.DAI, CURRENCY.ETH, wallet2.address, WeiPerEther.mul(550))
            ).to.be.revertedWith(ErrorDecoder.encodeError(ErrorCodes.INCORRECT_CASH_BALANCE));
            await expect(
                escrow
                    .connect(wallet2)
                    .settleCashBalance(CURRENCY.DAI, CURRENCY.ETH, owner.address, WeiPerEther.mul(550))
            ).to.be.revertedWith(ErrorDecoder.encodeError(ErrorCodes.INCORRECT_CASH_BALANCE));
            await expect(
                escrow.settleCashBalance(CURRENCY.DAI, CURRENCY.ETH, wallet2.address, WeiPerEther.mul(500))
            ).to.be.revertedWith(ErrorDecoder.encodeError(ErrorCodes.INCORRECT_CASH_BALANCE));
        });
    ...
    describe("settle cash, trading scenarios [4-6]", async () => {
        it("[4] settle cash should not touch assets when the account is undercollateralized", async () => {
            ...
            await escrow.settleCashBalance(CURRENCY.DAI, CURRENCY.ETH, wallet.address, debtBalance.mul(-1));
        ...
        it("[6] should settle cash between accounts when eth must be sold", async () => {
            ...
            expect(await escrow.cashBalances(CURRENCY.ETH, wallet.address)).to.equal(ethAmount.sub(ethPurchased));
    ...
    describe("settle cash w/ fCash scenarios", async () => {
        ...
        it("should not settle fCash if has liquidity tokens", async () => {
            ...
            await futureCash
                .connect(wallet)
                .addLiquidity(maturities[1], parseEther("5"), parseEther("10"), 0, 100_000_000, BLOCK_TIME_LIMIT);
            await fastForwardToMaturity(provider, maturities[0]);
        ...
        it("should settle fCash in local currency, only liquidator payments", async () => {
            ...
            const [, remaining] = await t.getfCashValue(parseEther("100"), settleAmount, maturities[2], maturities[1]);
    ...
    describe("settle cash w/ reserves", async () => {
        it("should not settle reserves if has cash", async () => {
            await escrow.connect(wallet).deposit(dai.address, parseEther("10"));
            await expect(escrow.settleReserve(wallet.address, CURRENCY.ETH)).be.revertedWith(
                ErrorDecoder.decodeError(ErrorCodes.ACCOUNT_HAS_COLLATERAL)
            );
        });
        ...
        it("should not settle reserves if has liquidity tokens", async () => {
            ...
            await futureCash
                .connect(wallet)
                .addLiquidity(maturities[0], parseEther("10"), parseEther("10"), 0, 100_000_000, BLOCK_TIME_LIMIT);
        ...
        it("should settle reserves in full", async () => {
            ...
            expect(await escrow.cashBalances(CURRENCY.DAI, wallet.address)).to.equal(0);
            expect(await escrow.cashBalances(CURRENCY.DAI, wallet2.address)).to.equal(
                parseEther("1000").sub(settleAmount)
            );
        });
        ...
        it("should settle reserves partially", async () => {
            ...
            expect(await escrow.cashBalances(CURRENCY.DAI, wallet.address)).to.equal(
                parseEther("50").sub(settleAmount)
            );
    ...
    describe("liquidate w/ fcash", async () => {
        it("should not liquidate fCash if collateralized", async () => {
            ...
            await expect(escrow.liquidatefCash(wallet.address, CURRENCY.DAI, CURRENCY.ETH)).be.revertedWith(
                ErrorDecoder.decodeError(ErrorCodes.CANNOT_LIQUIDATE_SUFFICIENT_COLLATERAL)
            );
        });
    ...
    describe("liquidation scenarios", async () => {
        ...
        it("[2] should recollateralize an account using just liquidity tokens before it touches eth", async () => {
            ...
            await futureCash
                .connect(wallet)
                .addLiquidity(maturities[1], parseEther("10"), parseEther("10"), 0, 100_000_000, BLOCK_TIME_LIMIT);
        ...
        it("[3] should liquidate an account when it is under collateralized by eth", async () => {
            ...
            const ethPurchased = daiFCChange.mul(newRate).mul(liquidationBonus).div(WeiPerEther).div(WeiPerEther);
        ...
        it("[3] should account for dai when partially liquidating an account", async () => {
            ...
            const ethPurchased = daiPurchased.mul(newRate).mul(liquidationBonus).div(WeiPerEther).div(WeiPerEther);
            ...
            expect(liquidatorDaiBalanceBefore.sub(daiPurchased).div(1e5)).to.equal(liquidatorDaiBalanceAfter.div(1e5));
        });
        ...
        it("[4] should liquidate an account up to a certain amount of DAI", async () => {
            ...
            const ethPurchased = maxDaiLiquidate.mul(newRate).mul(liquidationBonus).div(WeiPerEther).div(WeiPerEther);
            ...
            expect(ethBalanceBefore.sub(ethBalanceAfter).div(100), "ETH balances do not match").to.equal(
                ethPurchased.div(100)
            );
        });
    });
}).timeout(50000);
