...
describe("Liquidation Calculations", () => {
    ...
    beforeEach(async () => {
        ...
        libraries.set("Liquidation", (await NotionalDeployer.deployContract(owner, "Liquidation", [], 1)).contract);

        liquidation = (await NotionalDeployer.deployContract(owner, MockLiquidationArtifact, [], 1))
            .contract as MockLiquidation;

        portfolios = (await NotionalDeployer.deployContract(owner, MockPortfoliosArtifact, [], 1))
            .contract as MockPortfolios;

        chainlink = (await NotionalDeployer.deployContract(owner, MockAggregatorArtifact, [], 1))
    ...
    describe("localLiquidityTokenTrade", async () => {
        it("raises the full amount when no remainder at 18 decimals", async () => {
            ...
            await expect(
                liquidation.localLiquidityTokenTrade(
                    AddressZero,
                    CURRENCY.DAI,
                    parseEther("1000"),
                    defaultLiquidityHaircut,
                ...
                .withArgs(expectedRaise, parseEther("1000"));
    ...
    describe("calculate deposit to sell", async () => {
        it("base case", async () => {
            const value = await liquidation.calculateCollateralToSell(defaultLiquidationDiscount, parseEther("1000"), {
        ...
        it("small values, convert up", async () => {
            // When converting down from 18 decimals to 6 we cannot sell under some amount of dust
            const value = await liquidation.calculateCollateralToSell(defaultLiquidationDiscount, 0.0001e6, {
                rate: 0.01e6,
                localCurrency: 1,
                collateralCurrency: 2,
                localDecimals: 1e6,
    ...
    describe("transfer collateral currency", async () => {
        const runTest = async (
            ...
            if (revert) {
                await expect(
                    liquidation.tradeCollateralCurrency(
                        AddressZero,
